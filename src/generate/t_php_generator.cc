 ...
 * under the License.
 *
 *Apache Thrift Copyright 2006-2010 The Apache Software Foundation
 *Zan Thrift    Copyright 2016-2017 Zan Group   <zan@zanphp.io>
 */
...
#define SPECX                   "Servicespecification"
#define SERVICE_KEY_WORD        "service"
#define SERVICE_KEY_WORD_LEN    7
#endif
...
#ifdef ZAN
    void generate_const    (t_const*    tconst, std::ofstream& f_stream);
#endif
...
#ifdef  ZAN
            if (ns[position + 1] >= 'a' && ns[position  + 1] <= 'z') {
                ns[position + 1] = ns[position + 1] - 32;
            }
...
private:
     ...
     * Interface dir
     */
    std::string interface_dir_;
    /*
     * specx dir
     */
    std::string specx_dir_;
    /*
     * structs dir
     */
    std::string structs_dir_;
    /*
     * module name
     */
     ...
     * specx namespace
     */
    std::string specx_ns_;
    
    void gen_structs_header(std::ofstream& f_stream, bool is_exception = false);
    std::string structs_includes(bool is_exception = false);
    std::string client_includes();
    std::string services_includes();
    void generate_service_client(t_service* tservice, std::ofstream& f_stream);
    void generate_service_interface(t_service* tservice, std::ofstream& f_stream);
    void generate_service_service(t_service* tservice, std::ofstream& f_stream);
#endif
};
...
void t_php_generator::init_generator() {
    ...
    std::string strNameSpace = php_namespace_suffix(get_program());
    vector<string> NSx = split(strNameSpace, '\\');
    package_dir_ = get_out_dir();
    if (get_program()->get_services().size() != 0
        && strNameSpace.find("service") == std::string::npos
         && strNameSpace.find("Service") == std::string::npos) {
        printf("\033[1;31;40m[WARNING]\033[0m  service's namespace has no key word 'service'. [namespace=%s]\n", strNameSpace.c_str());
        exit(-1);
    }
...
//    MKDIR(package_dir_.c_str());
...
#ifdef ZAN
    //WARN 约定namespace必须是以下格式:
    //  com.company.modulename
    if (NSx.size() < 3 || (NSx[0] != std::string("com") && NSx[0] != std::string("Com"))) {
        std::cout << program_name_ <<": PHP's namespace must like com.${company}.${modulename}[.xxx]" << std::endl;
        exit(-1);
    }
    module_name_ = NSx[2];
    ...
    service_dir_ = package_dir_;// + SERVICE;
    interface_dir_ = package_dir_;//  + INTERFACES;
    specx_dir_ = package_dir_;//  + SPECX;
    structs_dir_ = package_dir_;
    
    if(get_program()->get_structs().size() != 0 || get_program()->get_enums().size() != 0 || get_program()->get_xceptions().size() != 0) {
        MKDIR(structs_dir_.c_str());
        for (size_t i = 3; i < NSx.size(); ++i) {
            //dir for structs
//            printf("%s\n", NSx[i].c_str());
            structs_dir_ = structs_dir_ + "/" + NSx[i] + "/";
            MKDIR(structs_dir_.c_str());
        }
    }
    if (get_program()->get_services().size() != 0) {
        
        MKDIR(service_dir_.c_str());
        MKDIR(interface_dir_.c_str());
        MKDIR(specx_dir_.c_str());
        
        for (size_t i = 3; i < NSx.size(); ++i) {
            if (NSx[i] == "service" || NSx[i] == "Service") {
                service_dir_ = service_dir_  + "/" + SERVICE + "/";
                interface_dir_ = interface_dir_  + "/" + INTERFACES + "/";
                specx_dir_ = specx_dir_  + "/" + SPECX + "/";
            } else {
//                if(NSx[i][0] >= 'a' && NSx[i][0] <= 'z')
//                {
//                    NSx[i][0] = NSx[i][0] - 32;
//                    
//                }
                service_dir_ = service_dir_  + "/" + NSx[i] + "/";
                interface_dir_ = interface_dir_  + "/" + NSx[i] + "/";
                specx_dir_ = specx_dir_  + "/" + NSx[i] + "/";
            }
            // mkdir for service
            MKDIR(service_dir_.c_str());
            //mkdir for interfaces
            MKDIR(interface_dir_.c_str());
            //mkdir for specx
            MKDIR(specx_dir_.c_str());
        ...
        size_t npos;
        service_ns_ = php_namespace_suffix(get_program());
        if((npos = service_ns_.find(SERVICE_KEY_WORD)) == string::npos
           && (npos = service_ns_.find(SERVICE)) == string::npos)
        {
            std::cout << program_name_ <<": when generate php service can not find the key word 'service' in namespace.[namespace="<<service_ns_<<"]. pls contact lanhu(zhangyuanhao@qima-inc.com)" << std::endl;
            exit(-1);
        }
        
        interface_ns_ = service_ns_;
        interface_ns_.replace(npos, SERVICE_KEY_WORD_LEN, INTERFACES);
        
        specx_ns_ = service_ns_;
        specx_ns_.replace(npos, SERVICE_KEY_WORD_LEN, SPECX);
    }
#else
...
#ifdef ZAN
    std::ofstream f_enum;
    std::string str_enum_file = structs_dir_ + tenum->get_name() + ".php";
    f_enum.open(str_enum_file.c_str());
    gen_structs_header(f_enum);
...
void t_php_generator::generate_consts(vector<t_const*> consts) {
    ...
    std::ofstream f_consts;
    std::string str_consts_file = structs_dir_ + program_name_ + "Constant.php";
    f_consts.open(str_consts_file.c_str());
    gen_structs_header(f_consts);
    
    // Create class only if needed
    if(consts.size() > 0)
    {
        f_consts << "final class "+program_name_ + "Constant extends \\Thrift\\Type\\TConstant {" << endl;
        indent_up();
        ...
        for (c_iter = consts.begin(); c_iter != consts.end(); ++c_iter) {
            ...
            generate_const(*c_iter, f_consts);
            f_consts << ";" << endl;
...
#ifdef ZAN
void t_php_generator::generate_const(t_const*    tconst, std::ofstream& f_stream)
{
    t_type* type = tconst->get_type();
    t_const_value* value = tconst->get_value();
    
    generate_php_doc(f_stream, tconst);
    f_stream << render_const_value(type, value);
}
#endif
...
void t_php_generator::generate_php_struct(t_struct* tstruct,
                                          bool is_exception) {
#ifdef ZAN
    std::string str_struct_file = structs_dir_ + tstruct->get_name() + ".php";
    std::ofstream f_struct;
    f_struct.open(str_struct_file.c_str());
    gen_structs_header(f_struct, is_exception);
...
void t_php_generator::generate_php_type_spec(ofstream& out,
    ...
    t = get_true_type(t);
    if (t->is_void()) {
        indent(out) << "'type' => TType::VOID," << endl;
    }
    else {
...
void t_php_generator::generate_php_struct_spec(ofstream& out,
    ...
    indent(out) << "protected $structSpec = [" << endl;
    indent_up();
    ...
    for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
        ...
        indent(out) << (*m_iter)->get_key() << " => [" << endl;
        indent_up();
        if ((*m_iter)->get_req() == t_field::T_REQUIRED) {
            indent(out) << "'required' => true,"<<endl;
        }
        out <<
...
#ifdef ZAN
    // generate client
    std::string f_client_name = service_dir_ + service_name_ + ".php";
    std::ofstream f_client(f_client_name.c_str());
    
    f_client << "<?php" << endl;
    f_client << autogen_comment() << endl;
    f_client << "namespace " << service_ns_ << ";" << endl;
    f_client << endl;
    f_client << client_includes();
    f_client << endl;
    
    f_client << "use " <<interface_ns_ << "\\" << service_name_ << " as " << service_name_ << "Interface;" << endl;
    f_client << "use " <<specx_ns_ << "\\" << service_name_ << " as " << service_name_ << "Specification;" << endl;
    f_client << endl;
    
    generate_service_client(tservice, f_client);
    
    f_client.close();
    
    // generate interface
    std::string f_interfaces_name = interface_dir_ + service_name_ + ".php";
    std::ofstream f_interfaces(f_interfaces_name.c_str());
    
    f_interfaces << "<?php" << endl;
    f_interfaces << autogen_comment() << endl;
    f_interfaces << "namespace " <<  interface_ns_ << ";" << endl;
    f_interfaces << endl;
    
    generate_service_interface(tservice, f_interfaces);
    
    f_interfaces.close();
    
    // generate services
    std::string f_services_name = specx_dir_ + service_name_ + ".php";
    std::ofstream f_services(f_services_name.c_str());
    
    f_services << "<?php" << endl;
    f_services << autogen_comment() << endl;
    f_services << "namespace " <<  specx_ns_ << ";" << endl;
    f_services << endl;
    f_services << services_includes();
    f_services << endl;
    
    
    generate_service_service(tservice, f_services);
...
void t_php_generator::generate_service_client(t_service* tservice) {
    ...
    f_service_ <<
#ifdef ZAN
    "class " << php_namespace_declaration(tservice) << extends_client << " implements " <<  php_namespace(tservice->get_program()) << service_name_ << "If {" << endl;
#else
...
string t_php_generator::argument_list(t_struct* tstruct, bool addTypeHints) {
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        ...
        if ((*f_iter)->get_value() != NULL && !(type->is_struct() || type->is_xception())) {
            string dval = render_const_value((*f_iter)->get_type(), (*f_iter)->get_value());
            result += "=" + dval;
        }
#endif
    }
...
#ifdef ZAN
void t_php_generator::gen_structs_header(std::ofstream& f_stream, bool is_exception)
{
    // Print header
    f_stream <<
    "<?php" << endl;
    f_stream << autogen_comment() << endl;
    
    f_stream << "namespace " << php_namespace_suffix(get_program()) << ";" << endl << endl;
    
    f_stream << structs_includes(is_exception);
 ...
 * Prints standard php includes
 */
string t_php_generator::structs_includes( bool is_exception) {
    string includes =
    "use Kdt\\Iron\\Nova\\Foundation\\Protocol\\TStruct;\n"
    "use Thrift\\Type\\TType;\n";
    if (is_exception) {
        includes += "use Kdt\\Iron\\Nova\\Foundation\\Protocol\\TException;\n";
    }
    return   includes + "\n";
}
 ...
 * Prints standard php includes
 */
string t_php_generator::client_includes() {
    string includes =
 ...
 * Prints standard php includes
 */
string t_php_generator::services_includes() {
    string includes =
...
void t_php_generator::generate_service_client(t_service* tservice, std::ofstream& f_stream) {
    string extends = "";
    string extends_client = "";
    if (tservice->get_extends() != NULL) {
        extends = tservice->get_extends()->get_name();
        ...
        extends_client = " extends TService";
    }
    
    f_stream <<
    "class " << php_namespace_declaration(tservice) << extends_client << " implements " << service_name_ << "Interface {" << endl;
    indent_up();
    
    //Generate client specificationProvider
    f_stream << endl;
    indent(f_stream) <<
    "protected function specificationProvider()" <<endl;
    scope_up(f_stream);
    indent(f_stream) <<
    "return new " << php_namespace_declaration(tservice) << "Specification();" << endl;
    scope_down(f_stream);
    f_stream << endl;
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        generate_php_doc(f_stream, *f_iter);
        // Open function
        indent(f_stream) <<
        "public function " << function_signature(*f_iter) << endl;
        scope_up(f_stream);
        indent(f_stream) <<
        "return $this->apiCall(__FUNCTION__, func_get_args());" <<endl;
        scope_down(f_stream);
        f_stream << endl;
    }
...
void t_php_generator::generate_service_interface(t_service* tservice, std::ofstream& f_stream) {
    string extends = "";
    string extends_if = "";
    if (tservice->get_extends() != NULL) {
        extends = " extends " + php_namespace(tservice->get_extends()->get_program()) + tservice->get_extends()->get_name();
        extends_if = " extends " + php_namespace(tservice->get_extends()->get_program()) + tservice->get_extends()->get_name();
    }
    generate_php_doc(f_stream, tservice);
    f_stream <<
    "interface " << php_namespace_declaration(tservice) << extends_if << " {" << endl;
    indent_up();
    vector<t_function*> functions = tservice->get_functions();
    vector<t_function*>::iterator f_iter;
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
...
void t_php_generator::generate_service_service(t_service* tservice, std::ofstream& f_stream)
{
    string extends = "";
    string extends_service = "";
    if (tservice->get_extends() != NULL) {
        extends = " extends " + php_namespace(tservice->get_extends()->get_program()) + tservice->get_extends()->get_name();
        extends_service = " extends " + php_namespace(tservice->get_extends()->get_program()) + tservice->get_extends()->get_name();
    }
    else {
        extends_service = " extends TSpecification";
    }
    generate_php_doc(f_stream, tservice);
    f_stream <<
    "class " << php_namespace_declaration(tservice) << extends_service << " {" << endl;
    indent_up();
    
    std::string str_service_name = php_namespace(tservice->get_program()) + tservice->get_name();
    std::string str_old_value = "\\";
    std::string str_new_value = ".";
    string::size_type   pos(0);
    for(pos = 0;   pos!=string::npos;   pos+=str_new_value.length())   {
        if((pos=str_service_name.find(str_old_value,pos))!=string::npos)
        {
            //去掉第一个"\\"
            if (pos == 0) {
                str_service_name.replace(pos,str_old_value.length(),std::string(""));
            } else {
                str_service_name.replace(pos,str_old_value.length(),str_new_value);
            }
        }
        else
            break;
    }
    //替换kdt.api为com.youzan
    pos = 0;
    if((pos=str_service_name.find(std::string("kdt.api"),pos))!=string::npos)
    {
        str_service_name.replace(pos,std::string("kdt.api").length(),std::string("com.youzan"));
    }
    f_stream << endl;
    indent(f_stream) << "protected $serviceName = '" << str_service_name << "';" << endl;
    f_stream << endl;
    
    indent(f_stream) << "protected $inputStructSpec = [" << endl;
    indent_up();
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        indent(f_stream) <<
        "'" << decapitalize((*f_iter)->get_name()) << "' => [" << endl;
        indent_up();
        //TODO
        ...
        for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
            t_type* t = get_true_type((*m_iter)->get_type());
            indent(f_stream) << (*m_iter)->get_key() << " => [" << endl;
            indent_up();
            f_stream <<
            indent() << "'var' => '" << (*m_iter)->get_name() << "'," << endl;
            generate_php_type_spec(f_stream, t);
            indent(f_stream) << "]," << endl;
            indent_down();
        }
    ...
    indent(f_stream) << "protected $outputStructSpec = [" << endl;
    indent_up();
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        indent_up();
        //TODO
        t_type* t = (*f_iter)->get_returntype();
    ...
    indent(f_stream) << "protected $exceptionStructSpec = [" << endl;
    indent_up();
    ...
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        ...
        if (xceptions.size() > 0) {
            ...
            //TODO
            for (x_iter = xceptions.begin(); x_iter!=xceptions.end(); ++x_iter) {
                t_type* t = get_true_type((*x_iter)->get_type());
                indent(f_stream) << (*x_iter)->get_key() << " => [" << endl;
                indent_up();
                f_stream <<
                indent() << "'var' => '" << (*x_iter)->get_name() << "'," <<endl;
                generate_php_type_spec(f_stream, t);
                indent(f_stream) << "]," << endl;
                indent_down();
            }
