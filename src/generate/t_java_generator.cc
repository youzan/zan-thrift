...
#ifdef ZAN
    void generate_java_struct_definition(std::ofstream& out, t_struct* tstruct, bool is_xception=false, bool in_class=false, bool is_result=false, bool is_args=false);
#else
...
#ifdef ZAN
    void generate_java_validator(std::ofstream& out, t_struct* tstruct, bool is_args=true);
#else
...
#ifdef ZAN
    void generate_service_void_interface(t_service *tservice, std::ofstream& f_stream);
    std::string void_function_signature(t_function* tfunction, string prefix="");
    /**
     * Split method
     * @param s
     * @param delim
...
#ifdef ZAN
    size_t pos(0);
    vector<string> NSx = split(package_name_, '.');
    if (NSx.size() < 3 || (NSx[0] != std::string("com") && NSx[0] != std::string("Com"))) {
        std::cout << program_name_ <<": Java's namespace must like com.${company}.${modulename}[.xxx]" << std::endl;
        exit(-1);
    }
#endif
    ...
    if (!members.empty() && !all_optional_members) {
        ...
        for (m_iter = members.begin(); m_iter != members.end(); ++m_iter) {
            if ((*m_iter)->get_req() != t_field::T_OPTIONAL) {
                t_type* type = get_true_type((*m_iter)->get_type());
#ifdef ZAN
                indent(out) << "if ("<<(*m_iter)->get_name()<<" != null ) {"<<endl;
                indent_up();
#endif
                if (type->is_base_type() && ((t_base_type*)type)->is_binary()) {
...
#ifdef ZAN
    generate_java_validator(out, tstruct, is_args);
#else
...
void t_java_generator::generate_java_validator(ofstream& out,
                                               t_struct* tstruct){
#endif
    out << indent() << "// check for args" << endl;
    ...
    if (is_args) {
        for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
            ...
            indent_up();
            indent(out) << "throw new org.apache.thrift.protocol.TProtocolException(\"Required field '" << (*f_iter)->get_name() << "' is unset! Struct:\" + toString());" << endl;
            indent_down();
            indent(out) << "}" << endl <<endl;
        }
    } else {
...
void t_java_generator::generate_reflection_getters(ostringstream& out, t_type* type, string field_name, string cap_name) {
    ...
    if (type->is_base_type() && !type->is_string()) {
        ...
        indent(out) << "if (" << (base_type->is_bool() ? "is" : "get") << cap_name <<  "() != null) {"<<endl;
        indent_up();
#endif
        indent(out) << "return " << type_name(type, true, false) << ".valueOf(" << (base_type->is_bool() ? "is" : "get") << cap_name << "());" << endl;
    } else {
#ifdef ZAN
        indent(out) << "if (get" << cap_name << "() != null) {"<<endl;
        indent_up();
#endif
...
void t_java_generator::generate_java_bean_boilerplate(ofstream& out,
    ...
    for (f_iter = fields.begin(); f_iter != fields.end(); ++f_iter) {
        ...
        indent(out) << "} else {"<<endl;
        indent_up();
        indent(out) << "unset" << cap_name << "();" << endl;
        indent_down();
        indent(out)<< "}" << endl;
#endif
        if (!bean_style_) {
...
void t_java_generator::generate_service(t_service* tservice) {
    ...
    string avatar_service_name = "Avatar"+service_name_;
    string f_avatar_service_name = package_dir_ + "/" + make_valid_java_filename(avatar_service_name) + ".java";
    f_avatar_service_.open(f_avatar_service_name.c_str());
    ...
    generate_javax_generated_annotation(f_avatar_service_);
    
    f_avatar_service_ << "public interface " << avatar_service_name << " {" << endl << endl;
    indent_up();
    
    generate_service_void_interface(tservice, f_avatar_service_);
    indent_down();
    f_avatar_service_ <<
    "}" << endl;
    f_avatar_service_.close();
    */
#else
...
#ifdef ZAN
        generate_java_struct_definition(f_service_, ts, false, true, true, true);
#else
        generate_java_struct_definition(f_service_, ts, false, true, true);
#endif
...
void t_java_generator::generate_serialize_container(ofstream& out,
    ...
    string isize = tmp("_isize");
#ifdef ZAN
    if (ttype->is_map()) {
        indent(out)<< "int "+isize+" = 0;" << endl;
        indent(out)<<"for (Map.Entry<" <<
        type_name(((t_map*)ttype)->get_key_type(), true, false) << ", " <<
        type_name(((t_map*)ttype)->get_val_type(), true, false) << "> " << iter <<
        " : " << prefix << ".entrySet())" << endl;
        out << endl;
        scope_up(out);
        
        indent(out) << "if ("+iter+" != null &&"+iter+".getValue() != null ) " << endl;
        scope_up(out);
        indent(out) << isize +"++;"<<endl;
...
//        return base_type_name((t_base_type*)ttype, in_container);
        return base_type_name((t_base_type*)ttype, true);
...
string t_java_generator::declare_field(t_field* tfield, bool init, bool comment) {
    ...
    if (comment) {
        ...
        if (tfield->get_req() == t_field::T_OPTIONAL) {
            result += "optional";
#ifdef ZAN
        } else if (tfield->get_req() == t_field::T_REQUIRED) {
            result += "required";
        } else {
            result += "default";
        }
...
#ifdef ZAN
void t_java_generator::generate_service_void_interface(t_service *tservice, std::ofstream& f_stream)
{
    vector<t_function*> functions = tservice->get_functions();
    vector<t_function*>::iterator f_iter;
    for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
        generate_java_doc(f_stream, *f_iter);
        indent(f_stream) << "public " << void_function_signature(*f_iter) << ";" << endl << endl;
    }
}

std::string t_java_generator::void_function_signature(t_function* tfunction, string prefix)
{
    std::string fn_name = get_rpc_method_name(tfunction->get_name());
    std::string result ="void " + prefix + fn_name + "(" + argument_list(tfunction->get_arglist()) + ")";
    return result;
}
#endif
