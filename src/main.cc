...
void version() {
    ...
    printf("For ZAN_THRIFT version %s\n", ZAN_THRIFT_VERSION);
}
...
void usage() {
#ifdef ZAN
    fprintf(stderr, "Usage: zan-thrift [options] file\n\n");
    fprintf(stderr, "Use zan-thrift -help for a list of options\n");
#else
...
vector<string> split(const string &s, char delim) {
    ...
    bool isPathAndNameSpaceValid(std::string& strPath, std::string& strNameSpace)
    {
        vector<std::string> lsNameSpace = split(strNameSpace, '.');
        vector<std::string> lsPath = split(strPath, '/');
        // why namespace = path + 3?
        //namespace: com.${company}.${modulename}[.xxx]
        //path: xxx/xxx/xxx.thrift
        if (lsNameSpace.size() != lsPath.size() + 2) {
            return false;
        }
        for (int i=3, j=0; i<lsNameSpace.size() - 1  && j<lsPath.size() - 1; ) {
            if (lsNameSpace[i] != lsPath[j]) {
                return false;
            }
            i++;
            j++;
    ...
    int main_zan(int argc, char** argv)
    {
        int i;
        std::string out_path;
        bool out_path_is_absolute = false;
        list<string> lsFailed;
        ...
        bzero(szCurPath, sizeof(szCurPath));
        if (getcwd(szCurPath, MAX_PATH_LEN) == NULL) {
            printf("\033[1;31;40m[WARNING]\033[0m  get working dir failed. [%s]\n", strerror(errno));
            return -1;
        }
        
        char* pTmp = NULL;
        if ((pTmp = strrchr(szCurPath, '/')) != NULL) {
            int nLen = abs((long)(pTmp - szCurPath));
            out_path = std::string(szCurPath, nLen);
            out_path += "/sdk";
#ifdef _WIN32
...
#endif
            if (mkdir(out_path.c_str(), 0755) == -1) {
                if (errno != EEXIST) {
                    printf("\033[1;31;40m[WARNING]\033[0m  mkdir failed.[path=%s, error=%s]\n", out_path.c_str(), strerror(errno));
                    return -1;
                }
            }
        ...
        if (generator_strings.empty()) {
            generator_strings.push_back("php");
            generator_strings.push_back("java");
        }
        
        CFileList fileList;
        fileList.Parse(szCurPath);
        std::list<std::string> lsFiles = fileList.getFileList();
        
        if (lsFiles.size() == 0) {
            printf("\033[1;31;40m[WARNING]\033[0m  no thrift file.\n");
            return -1;
        }
        

        std::list<std::string>::iterator it=lsFiles.begin();
        for (; it!= lsFiles.end(); ++it) {
            printf("\033[0;32;40mProcessing:\033[0m %s\n", it->c_str());
            // Real-pathify it
            char rp[THRIFT_PATH_MAX];
            
            if (saferealpath(it->c_str(), rp) == NULL) {
            ...
            yylineno = 1;
            //先对namespace跟目录结构进行判断
            std::string strPath(it->c_str() + strlen(szCurPath) +1);
            std::string strNameSpace = program->get_namespace("*");
            if (program->get_services().size() != 0 &&
                !isPathAndNameSpaceValid(strPath, strNameSpace)) {
                std::cout << "\033[1;31;40m[WARNING]\033[0m \033[0m path and namespace is not match.[path=" << strPath.c_str() << ", namespace="<< strNameSpace.c_str() << "]\n" << std::endl;
                lsFailed.push_back(*it);
                continue;
            }
        ...
        if (lsFailed.size() != 0) {
            std::cout << "\033[1;31;40m Failed:\033[0m" << std::endl;
            for (list<string>::iterator it=lsFailed.begin(); it!=lsFailed.end(); ++it) {
