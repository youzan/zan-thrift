...
void binary_deserialize(int8_t thrift_typeID, PHPInputTransport& transport, zval* return_value, HashTable* fieldspec) {
  ...
  switch (thrift_typeID) {
    ...
    case T_STRUCT: {
      ...
      TSRMLS_FETCH();
      zval* spec = zend_read_property(zend_get_class_entry(return_value TSRMLS_CC), return_value, "_TSPEC", 6, false TSRMLS_CC);
...
void binary_serialize(int8_t thrift_typeID, PHPOutputTransport& transport, zval** value, HashTable* fieldspec) {
  ...
  switch (thrift_typeID) {
    ...
    case T_STRUCT: {
      ...
      zval* spec = zend_read_property(zend_get_class_entry(*value TSRMLS_CC), *value, "_TSPEC", 6, false TSRMLS_CC);
      if (Z_TYPE_P(spec) != IS_ARRAY) {
...
PHP_FUNCTION(thrift_protocol_write_binary) {
  ...
  try {
    ...
    zval* spec = zend_read_property(zend_get_class_entry(request_struct TSRMLS_CC), request_struct, "_TSPEC", 6, false TSRMLS_CC);
    if (Z_TYPE_P(spec) != IS_ARRAY) {
...
PHP_FUNCTION(thrift_protocol_read_binary) {
  ...
  if (Z_TYPE_PP(args[1]) != IS_OBJECT) {
    php_error_docref(NULL TSRMLS_CC, E_ERROR, "2nd parameter is not an object (response struct)");
    efree(args);
    RETURN_NULL();
  }
  ...
  try {
    ...
    if (messageType == T_EXCEPTION) {
      ...
      zval* spec = zend_read_property(zend_get_class_entry(ex TSRMLS_CC), ex, "_TSPEC", 6, false TSRMLS_CC);
      binary_deserialize_spec(ex, transport, Z_ARRVAL_P(spec));
      throw PHPExceptionWrapper(ex);
    }

    zval* spec = zend_read_property(zend_get_class_entry(response_struct TSRMLS_CC), response_struct, "_TSPEC", 6, false TSRMLS_CC);
    binary_deserialize_spec(response_struct, transport, Z_ARRVAL_P(spec));
  } catch (const PHPExceptionWrapper& ex) {
    zend_throw_exception_object(ex TSRMLS_CC);
    RETURN_NULL();
  } catch (const std::exception& ex) {
